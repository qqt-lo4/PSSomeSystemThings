function Invoke-ScheduledTask {
    <#
    .SYNOPSIS
        Executes a script block via Windows Task Scheduler

    .DESCRIPTION
        Registers and runs a PowerShell script block as a scheduled task, supporting
        execution as SYSTEM, a specific user, a GMSA account, or interactively.
        Handles $Using: variables and supports elevated execution.
    ...
    .PARAMETER AsUser
        PSCredential to run the task under a specific user account.

    .PARAMETER AsSystem
        Run the task as NT AUTHORITY\SYSTEM.

    .PARAMETER AsInteractive
        Username to run the task interactively as.

    .PARAMETER AsGMSA
        Group Managed Service Account to run the task as.

    .PARAMETER RunElevated
        Run the task with highest privileges.

    .OUTPUTS
        The output of the executed script block.

    .EXAMPLE
        Invoke-ScheduledTask -ScriptBlock { Get-Process } -AsSystem

    .EXAMPLE
        Invoke-ScheduledTask -ScriptBlock { param($x) $x * 2 } -ArgumentList 5 -RunElevated
